...
public class ManagerSencePKFriend : MonoBehaviour 
{
	...
	/// 是否复仇
	private static bool _isRevenge;
	...
	/// 开始战斗
	public static void start(uint idServerPlayer, bool sIsRevenge = false)
	{
		/// 场景锁定的时候，什么都不要干
...
//		SuperUI.show<UIPKResult>("GusUIPKResult", urls.ToArray());
	...
	private static void assetLoad()
	{
		...
		ConfigUrl.getAssetsServerHero(urls, DataMode.getPlayer(_idServerPlayer).infoHeroList.getTeam());
		ConfigUrl.getAssetsServerBeast(urls, DataMode.getPlayer(_idServerPlayer).infoHeroList.getTeamBeast(DataMode.getPlayer(_idServerPlayer).infoHeroList.idTeamSelect));
	...
	private static void complete(double loader_id)
	{
		...
		createAICombat();
		if(_isRevenge)
			ManagerCombat.startPKFriendRevengeCombat();
		else
			ManagerCombat.startPKFriendCombat();
	}
	...
	private static void createBeast()
	{
		...
		if(null != DataMode.getPlayer(_idServerPlayer).infoHeroList.getTeamBeast(DataMode.getPlayer(_idServerPlayer).infoHeroList.idTeamSelect))
		{
			ManagerCombatBeast.beastEnemy = ManagerCreate.createBeast(DataMode.getPlayer(_idServerPlayer).infoHeroList.getTeamBeast(DataMode.getPlayer(_idServerPlayer).infoHeroList.idTeamSelect).idServer, 13);
		}
	}
	/// 创建敌人
	private static void createAICombat()
	{
		...
		/// 敌人家的站位信息
		{
			...
			/// 计算站位
			player.infoHeroList.mathStandIndex(player.infoHeroList.idTeamSelect);
			/// 我的出战团队
			List<InfoHero> infoHeroList = player.infoHeroList.getTeam();
	...
	{
		DataMode.myPlayer.isInPKFriend = true;
